...
class Fusion(object):
	...
	def hasExonInfo(self):
		if (len(self.lastExon5Gene) > 0 and len(self.firstExon3Gene) > 0):
			return True
		return False
		...
		for line in f:
			...
			if (fields[0]!="SAMPLE_ID"):
				...
				if(fusion_id in fusion_ids.keys()):
					...
					if(fusion_id in fusion_info.keys() and fusion_info[fusion_id].hasExonInfo() is False):
						print("HERE")
